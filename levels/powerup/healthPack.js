class HealthPack extends EngineObject {
    constructor(pos) {
        super(pos, vec2(1, 1), undefined, 0, new Color(0, 1, 0)); // Increased initial size
        ...
        this.sprite.src = '../../sprites/powerups/healthpack.gif'; // Update with the correct path
        ...
        this.framesPerRow = 1; // Number of frames in a row in the sprite sheet
        this.totalFrames = 1; // Total number of frames in the sprite sheet (adjust as needed)

        // Pulse scaling properties
        this.pulseTime = 0; // Time tracker for pulse effect
    }

    update() {
        super.update();

        // Update pulse animation for scaling effect
        this.pulseTime += 0.075; // Increase for faster pulsing
        const scaleMultiplier = 1 + Math.sin(this.pulseTime) * 0.1; // Increased scaling effect

        // Apply pulse scaling to size
        this.size = vec2(1, 1).scale(scaleMultiplier); // Initial size is now (1, 1)
    ...
    render() {
        ...
        if (this.sprite.complete && this.sprite.naturalWidth !== 0) {
            ...
            // Draw the animated sprite with pulse scaling
            context.drawImage(
                this.sprite,
